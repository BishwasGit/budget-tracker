...
<div class="px-4 py-6 sm:px-0">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('transactions.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md font-medium">New Transaction</a>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($transactions as $transaction)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ number_format($transaction->paid_amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${{ number_format($transaction->remaining_amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if($transaction->status !== 'completed')
                                    <button onclick="openPaymentModal({{ $transaction->id }}, {{ $transaction->remaining_amount }})"
                        ...
                        @if($transaction->description)
                            <tr>
                                <td></td>
                                <td colspan="8" class="px-6 py-2 text-sm text-gray-600 italic">{{ $transaction->description }}</td>
                            </tr>
                        @endif
                    @empty
                        <tr>
        ...
        <div class="mt-3 text-center">
            ...
            <form id="paymentForm" method="POST" class="mt-4">
                ...
                <div class="flex items-center justify-center space-x-4">
                    ...
                    <button type="submit"
                            class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Mark Paid</button>
                </div>
            </form>
        </div>
    </div>
...
function openPaymentModal(transactionId, remainingAmount) {
    ...
    document.getElementById('paymentForm').action = `/transactions/${transactionId}/mark-paid`;
